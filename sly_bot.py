...
# ==========================================
@bot.tree.command(name="wwsd", description="Costs 5 booms! Pull a random past message from this member.")
async def wwsd(interaction: discord.Interaction):
    ...
    if user_id not in boom_data or boom_data[user_id]["total_booms"] < 5:
        ...
        await interaction.response.send_message(
            f"❌ **ACCESS DENIED.** The Oracle demands payment.\n"
    ...
    for channel in interaction.guild.text_channels:
        try:
            async for message in channel.history(limit=250):
                if message.author.id == TARGET_MEMBER_ID and message.content:
                    if not message.content.startswith("/wwsd"):
    ...
    if valid_messages:
        ...
        display_name = chosen_message.author.display_name
        random_intro = random.choice(INTROS).format(name=display_name)

        response = (
            f"**{random_intro}**\n"
            f'"{chosen_message.content}"\n\n'
            f"🔗 [Jump to original message]({chosen_message.jump_url})\n\n"
            f"💸 *[Transaction: -5 Booms deducted. Remaining Wallet Balance: {remaining_balance} Booms]*"
        )
        
        if random.random() < 0.10:
            boom_data[user_id]["total_booms"] += 5
            # Since this is technically a refund, we don't count it as a brand new payout on career earnings
            save_boom_data(boom_data)
            response += (
